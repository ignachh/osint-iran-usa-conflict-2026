...
DATA_INIZIO = datetime(2026, 3, 5, 7, 0, 0, tzinfo=tz_cet)  # 5 marzo, ore 07:00
DATA_FINE = datetime(2026, 3, 11, 0, 0, 0, tzinfo=tz_cet)   # Fine del 10 marzo (mezzanotte dell'11)
STEP_ORE = 8                                                # Finestre da 8 ore (3 estrazioni al giorno)
MAX_TWEETS_PER_FASCIA = 500                                 # Obiettivo per ogni fascia oraria (Top Tweets)
...
async def raccogli_tweet_per_fascia(page, query_base, inizio_fascia, fine_fascia, max_tweets, nome_file_csv):
    ...
    while tweets_raccolti_fascia < max_tweets:
        ...
        if scroll_senza_nuovi >= 5:
            print("    Nessun nuovo tweet rilevante dopo 5 scroll. Raggiunto il limite dei Top Tweets per questa fascia.")
            break
...
async def gestore_raccolta(playwright, query_base, nome_corpus, file_output):
    ...
    while finestra_corrente_inizio < DATA_FINE:
        ...
        totale_fascia = await raccogli_tweet_per_fascia(
            page, 
            query_base, 
            finestra_corrente_inizio, 
            finestra_corrente_fine, 
            MAX_TWEETS_PER_FASCIA, 
            file_output
        )
...
async def main():
    async with async_playwright() as p:
        await gestore_raccolta(p, QUERY_BASE_ITALIA, "CORPUS ITALIA", "corpus_italia_stratif.csv")
